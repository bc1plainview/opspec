 ...
 */
function verifyAccess(accessSpec, method, sourceFile) {
    ...
    if (level === 'deployer-only') {
        ...
        const hasDeployerCheck = bodyNoComments.includes('onlyDeployer') ||
        ...
        const hasSenderCheck = bodyNoComments.includes('Blockchain.tx.sender') &&
        ...
        return makeResult(accessSpec, 'VIOLATED', `Method ${accessSpec.methodName}() is specified as deployer-only but has no onlyDeployer() call`, 'Add this.onlyDeployer(Blockchain.tx.sender) at the start of the method');
    }
    if (level === 'owner-only') {
        const bodyNoComments = stripComments(bodyText);
        const hasOwnerCheck = bodyNoComments.includes('ensureOwner') ||
        ...
        const hasManualOwnerCheck = bodyNoComments.includes('this.owner.value') &&
        ...
        return makeResult(accessSpec, 'VIOLATED', `Method ${accessSpec.methodName}() is specified as owner-only but has no owner check`, 'Add this.ensureOwner() or owner comparison at the start of the method');
    }
    if (level === 'anyone') {
        const hasRestriction = bodyText.includes('onlyDeployer') ||
            bodyText.includes('ensureOwner') ||
            bodyText.includes('onlyOwner');
        if (hasRestriction) {
            return makeResult(accessSpec, 'VIOLATED', `Method ${accessSpec.methodName}() is specified as @access anyone but has access restriction`);
        }
 ...
 */
function verifyPrecondition(preSpec, method, sourceFile) {
    ...
    if (expr.includes('status') || expr.includes('STATUS') || expr.includes('state')) {
        ...
        for (const helper of guardHelpers) {
            if (bodyText.includes(helper)) {
                return makeResult(preSpec, 'UNVERIFIED', `Precondition "${expr}" may be enforced by ${helper}() call — needs deeper call-graph analysis`);
            }
        }
    }
    ...
    for (const guard of guards) {
        ...
        if (matchCount > 0 && matchCount >= Math.ceil(keyTerms.length * 0.5)) {
            return makeResult(preSpec, 'VERIFIED', `Precondition "${expr}" matched by guard: ${guard.condition}`);
        }
    }
    ...
    if (bodyHasFields && bodyHasRevert) {
        return makeResult(preSpec, 'UNVERIFIED', `Method references fields from precondition "${expr}" and has Revert, but exact guard match not confirmed`);
    }
    if (bodyHasRevert) {
        return makeResult(preSpec, 'UNVERIFIED', `Method has Revert checks but couldn't match to precondition: ${expr}`);
    }
    return makeResult(preSpec, 'VIOLATED', `No guard check found for precondition: ${expr}`, 'Add an if-check that throws Revert when the precondition is violated');
}
/**
 ...
 */
function verifyPostcondition(postSpec, method, sourceFile, storedFields) {
    ...
    if (postSpec.fieldReferences.length > 0) {
        ...
        if (unmodified.length > 0) {
            return makeResult(postSpec, 'UNVERIFIED', `Postcondition references fields [${unmodified.join(', ')}] that are not modified in ${postSpec.methodName}()`, 'Ensure the method modifies these fields or update the postcondition');
        }
    }
    ...
    if (postSpec.expression.includes('return')) {
        ...
        if (!hasReturn) {
            return makeResult(postSpec, 'VIOLATED', `Postcondition references return value but method ${postSpec.methodName}() has no return statement`);
        }
    }
    // old() references need symbolic execution — mark as UNVERIFIED
    if (postSpec.oldReferences.length > 0) {
        return makeResult(postSpec, 'UNVERIFIED', `Postcondition uses old() references — requires symbolic execution for full verification`, `Fields referenced via old(): ${postSpec.oldReferences.join(', ')}`);
    }
    // If we get here, the postcondition looks structurally consistent
    if (postSpec.fieldReferences.length > 0) {
        return makeResult(postSpec, 'UNVERIFIED', `Postcondition "${postSpec.expression}" — fields are modified but value correctness requires symbolic execution`);
    }
    return makeResult(postSpec, 'UNVERIFIED', `Postcondition "${postSpec.expression}" — structural check passed but full verification requires symbolic execution`);
}
/**
 * Verify @ensures CEI — Checks-Effects-Interactions pattern.
 */
function verifyCEI(spec, method, sourceFile, storedFields) {
    const externalCalls = (0, ast_utils_1.findBlockchainCalls)(method, sourceFile);
    const stateWrites = (0, ast_utils_1.findStateWrites)(method, sourceFile, storedFields);
    ...
    if (violations.length > 0) {
        return makeResult(spec, 'VIOLATED', `CEI violation in ${spec.methodName}(): ${violations.join('; ')}`, 'Move all state writes before external calls');
    }
 ...
 */
function verifyCalls(callSpec, method, sourceFile) {
    const bodyText = (0, ast_utils_1.getMethodBodyText)(method, sourceFile);
    const externalCalls = (0, ast_utils_1.findBlockchainCalls)(method, sourceFile);
    if (externalCalls.length === 0) {
        return makeResult(callSpec, 'VIOLATED', `${callSpec.methodName}() specifies @calls ${callSpec.target} but has no Blockchain.call()`);
    }
    ...
    if (!targetReferenced) {
        return makeResult(callSpec, 'VIOLATED', `${callSpec.methodName}() specifies @calls to ${callSpec.target} but doesn't reference that target`);
    }
    ...
    if (calledMethodName) {
        ...
        const selectorRef = bodyText.includes(calledMethodName) ||
            bodyText.includes(`Selector`) ||
            bodyText.includes(`encodeSelector`);
        if (!selectorRef) {
            return makeResult(callSpec, 'UNVERIFIED', `${callSpec.methodName}() calls ${callSpec.target} but couldn't verify method '${calledMethodName}' is being called`);
        }
    }
    // Check must-succeed expectation
    if (callSpec.expectation === 'must-succeed') {
        const checksSuccess = bodyText.includes('.success') ||
            bodyText.includes('result.success') ||
            bodyText.includes('Revert');
        if (!checksSuccess) {
            return makeResult(callSpec, 'VIOLATED', `${callSpec.methodName}() specifies @calls -> must-succeed but doesn't check call result`, 'Check result.success and throw Revert on failure');
        }
    }
    return makeResult(callSpec, 'VERIFIED', `${callSpec.methodName}() calls ${callSpec.target} with result checking`);
}
/**
 ...
 */
function verifyStateTransition(stateSpec, method, sourceFile) {
    const bodyText = (0, ast_utils_1.getMethodBodyText)(method, sourceFile);
    ...
    if (!hasStateRef) {
        return makeResult(stateSpec, 'UNVERIFIED', `State transition ${fromState} -> ${toState} in ${transition.methods.join(', ')}(): no state-related field references found`);
    }
    // Check for guard (from-state check)
    const hasGuard = bodyText.includes('Revert') || bodyText.includes('throw');
    if (!hasGuard) {
        return makeResult(stateSpec, 'UNVERIFIED', `State transition ${fromState} -> ${toState}: method has state references but no guard/revert for invalid states`);
    }
    ...
    if (transition.condition) {
        ...
        if (!conditionMatched) {
            return makeResult(stateSpec, 'UNVERIFIED', `State transition condition "${transition.condition}" not found in method body`);
        }
    }
    // If from == to (self-transition), the state shouldn't change
    if (cleanFromState === cleanToState && isNegatedFrom === isNegatedTo) {
        return makeResult(stateSpec, 'UNVERIFIED', `Self-transition ${fromState} -> ${toState}: method has state guards but self-transition verification requires deeper analysis`);
    }
    return makeResult(stateSpec, 'UNVERIFIED', `State transition ${fromState} -> ${toState}: structural checks pass but full state machine verification requires deeper analysis`);
}
/**
 ...
 */
function verifyInvariant(invariant, classDecl, sourceFile, allMethods) {
    const results = [];
    if (invariant.fieldReferences.length === 0) {
        results.push(makeResult(invariant, 'UNVERIFIED', `Invariant "${invariant.expression}" references no this.X.value fields — can't verify statically`));
        return results;
    }
    ...
    if (modifyingMethods.length === 0) {
        results.push(makeResult(invariant, 'VERIFIED', `Invariant "${invariant.expression}" — no methods modify the referenced fields (trivially maintained)`));
        return results;
    }
    ...
    for (const methodName of modifyingMethods) {
        ...
        if (hasChecks) {
            results.push(makeResult(invariant, 'UNVERIFIED', `Invariant "${invariant.expression}" — ${methodName}() modifies fields [${invariant.fieldReferences.join(', ')}] and has guards, but full invariant maintenance requires symbolic execution`));
        }
        else {
            results.push(makeResult(invariant, 'UNVERIFIED', `Invariant "${invariant.expression}" — ${methodName}() modifies fields [${invariant.fieldReferences.join(', ')}] with NO guard checks`, `Consider adding validation to maintain the invariant in ${methodName}()`));
        }
    }
    return results;
}
 ...
 */
function verifyOpnetConstraint(spec, classDecl, sourceFile) {
    ...
    switch (spec.constraint) {
        case 'selectors-sha256': {
            ...
            if (hasHardcodedSelector && !hasEncodeSelector) {
                return makeResult(spec, 'VIOLATED', `Contract uses hardcoded selectors instead of encodeSelector() (SHA256)`, 'Replace hardcoded selectors with encodeSelector("methodName")');
            }
        ...
        case 'address-two-params': {
            ...
            if (violations.length > 0) {
                return makeResult(spec, 'VIOLATED', `Address.fromString() called with single parameter: ${violations.join(', ')}`, 'Use Address.fromString(str, network) with two parameters');
            }
        ...
        case 'no-approve': {
            ...
            if (hasApprove) {
                return makeResult(spec, 'VIOLATED', `Contract uses .approve() — should use increaseAllowance() instead`, 'Replace .approve() calls with .increaseAllowance()');
            }
        ...
        case 'csv-timelocks': {
            ...
            return makeResult(spec, 'UNVERIFIED', `CSV timelock verification requires Bitcoin script analysis — not available in static checker`);
        }
        default:
 ...
 */
function verifyContract(contract, sourceFile) {
    const results = [];
    ...
    for (const method of (0, ast_utils_1.getClassMethods)(classDecl)) {
        allMethods.set((0, ast_utils_1.getMethodName)(method), method);
    }
    ...
    for (const [methodName, methodSpecs] of contract.methods) {
        ...
        for (const temp of methodSpecs.temporal) {
            results.push(makeResult(temp, 'UNVERIFIED', `Temporal property "${temp.condition}" requires runtime monitoring — not verifiable statically in V1`));
        }
    }
    ...
    for (const st of contract.stateTransitions) {
        ...
        const alreadyVerified = results.some((r) => r.spec === st);
        if (alreadyVerified)
            continue;
 ...
 */
function verifyFile(filePath, specTree) {
...
// Helper Functions
// ============================================================================
function makeResult(spec, status, message, details) {
    return {
        spec,
        status,
        message,
...
function buildReport(contract, results) {
...
function extractFieldRefsFromExpr(expr) {
    const refs = [];
    const re = /this\.(\w+)/g;
    let m;
